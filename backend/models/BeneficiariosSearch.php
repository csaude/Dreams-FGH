...
class BeneficiariosSearch extends Beneficiarios
{
    ...
    {
        $query = Beneficiarios::find()->where(['emp_status'=>1])->andWhere(['NOT IN','district_code',[7,18,26,27,29,30]]);
		if (isset(Yii::$app->user->identity->role)&&(Yii::$app->user->identity->role>0)) { 
            ...
            $query = Beneficiarios::find()->where(['provin_code'=>$prov])->andWhere(['NOT IN','district_code',[7,18,26,27,29,30]])->andWhere(['emp_status'=>1]);
...
$query = Beneficiarios::find()->andWhere(['NOT IN','district_code',[7,18,26,27,29,30]])->andWhere(['emp_status'=>1]);

}

} else {
$query = Beneficiarios::find()->where(['emp_status'=>1])->andWhere(['NOT IN','ditrict_code',[7,18,26,27,29,30]]);
}
